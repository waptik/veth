 ...
 * @see https://github.com/documenso/documenso/blob/main/packages/lib/errors/app-error.ts#L209
 */
...
export class AppError extends Error {
    ...
    static parseError(error: any): AppError {
        ...
        if (error?.name === "TRPCClientError") {
            const parsedJsonError = AppError.parseFromJSON(
                error.data?.appError,
            );
        ...
        const validCode: string | null = typeof code === "string"
            ? code
            : AppErrorCode.UNKNOWN_ERROR;
        const validMessage: string | undefined = typeof message === "string"
            ? message
            : undefined;
     ...
     */
    static toJSON(
        { code, message, userMessage, statusCode }: AppError,
    ): TAppErrorJsonSchema {
